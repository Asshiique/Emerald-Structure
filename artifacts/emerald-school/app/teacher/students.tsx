  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#F5F4F2" }}>
      <View style={[styles.header, { paddingTop: isWeb ? 67 : insets.top }]}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.headerTitle}>Class {classSection}</Text>
          <Text style={styles.headerSub}>{students.length} students</Text>
        </View>
        <TouchableOpacity style={styles.addBtn} onPress={() => router.push("/teacher/add-student")}>
      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 16, paddingBottom: insets.bottom + 32 }} showsVerticalScrollIndicator={false}>
        {students.length === 0 ? (
          <View style={styles.empty}>
            ...
            <TouchableOpacity style={styles.addStudentBtn} onPress={() => router.push("/teacher/add-student")}>
              <Text style={styles.addStudentBtnText}>Add First Student</Text>
            </TouchableOpacity>
          </View>
        ) : (
          students.map((s) => (
...
const styles = StyleSheet.create({
  ...
  addStudentBtn: { backgroundColor: "#C0282A", paddingHorizontal: 20, paddingVertical: 10, borderRadius: 12 },
  addStudentBtnText: { fontSize: 14, fontWeight: "700", color: "#FFFFFF" },
});
