...
function SectionHeader({ label }: { label: string }) {
  return <Text style={ss.sectionHeader}>{label}</Text>;
}
  ...
  const handleSubmit = async () => {
    ...
    try {
      const student = await addStudent({
        ...
        prevSchool: form.prevSchool.trim(),
      });
      Alert.alert(
        "Student Added",
        ...
        <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: insets.bottom + 32 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          <View style={ss.card}>
            ...
            <View style={ss.infoBox}>
              ...
              <Text style={ss.infoText}>Parent will receive login credentials: their email + default password <Text style={{ fontWeight: "700" }}>Emerald@123</Text></Text>
            </View>
            ...
            <TouchableOpacity style={ss.btn} onPress={handleSubmit} disabled={loading} activeOpacity={0.85}>
              {loading ? <ActivityIndicator color="#FFFFFF" /> : (
                <>
                  ...
                  <Text style={ss.btnText}>Add Student</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
